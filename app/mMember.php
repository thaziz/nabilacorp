...
class mMember extends Model implements AuthenticatableContract, CanResetPasswordContract {
    ...
    static function perusahaan(){        
        return  DB::table('d_mem_comp')
                ->join('m_comp','mc_comp','=','c_id')
                ->where('mc_mem',Auth::user()->m_id)->get();
    }
    static function gudangCabang($namaGudang){
        return  DB::table('d_mem_gudangcomp')
                ->join('d_gudangcabang','gc_id','=','mg_gudangcomp')
                ->join('m_gudang','g_id','=','gc_gudang')
                ->whereIn('g_name',$namaGudang)
                ->where('mg_mem',Auth::user()->m_id)->select('mg_gudangcomp')->get();   
    }
