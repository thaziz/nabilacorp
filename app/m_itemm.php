...
class m_itemm extends Model
{
        ...
        if (count($sql)==0) {
          ...
          foreach ($sql as $data)
          {
            $results[] = [
                    ...
                    'm_isup_price1'  => number_format($data->m_isup_price1,0,',','.'),
                    'm_isup_price2'  => number_format($data->m_isup_price2,0,',','.'),
                    'm_isup_price3'  => number_format($data->m_isup_price3,0,',','.'),
                    'stok'     => number_format($data->s_qty,0,',','.'),
                    'i_code'   => $data->i_code,
                    'item'     => $data->i_name];
          }
    ...
    public static function searchItemTitipan($item) {
        ...
        if (count($sql)==0) {
          ...
          foreach ($sql as $data)
          {
            $results[] = ['label' => $data->i_name.'  (Rp. ' .number_format($data->is_price1,0,',','.').')', 'i_id' => $data->i_id,'satuan' =>$data->s_name,'stok' =>number_format($data->s_qty,0,',','.'),'i_code' =>$data->i_code,'i_price' =>number_format($data->is_price1,0,',','.'),'item' => $data->i_name, 'position'=>$position,  
              'comp'=>$comp];
          }
        } 
        return Response::json($results);
