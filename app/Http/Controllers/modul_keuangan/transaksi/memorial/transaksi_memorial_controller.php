...
namespace App\Http\Controllers\modul_keuangan\transaksi\memorial;
...
class transaksi_memorial_controller extends Controller
{
    public function index(){
    	return view('modul_keuangan.transaksi.memorial.index');
    }
    ...
    public function store(Request $request){
        ...
        try {
            ...
            $tr_number = 'TRANS-MM'.date('y', strtotime($date)).'/'.date('m', strtotime($date)).date('d', strtotime($date)).'/'.str_pad($num, 4, "0", STR_PAD_LEFT);
    ...
    public function update(Request $request){
        ...
        try {
            ...
            if(date('Y-m', strtotime($trans->first()->tr_tanggal)) != date('Y-m', strtotime($date))){
                ...
                $tr_number = 'TRANS-MM'.date('y', strtotime($date)).'/'.date('m', strtotime($date)).date('d', strtotime($date)).'/'.explode('/', $trans->first()->tr_nomor)[2];
            }
