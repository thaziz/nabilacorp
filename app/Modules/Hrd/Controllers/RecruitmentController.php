...
use App\d_pelamar;
use App\d_pelamar_jadwal;
use App\d_berkas_pelamar;
use App\d_cv_pelamar;
...
class RecruitmentController extends Controller
{
    ...
    public function rekrut()
    {
        ...
        $lulusan = ['SD', 'SMP', 'SMA', 'SMK', 'D1','D2', 'D3','D4', 'S1', 'S2', 'S3'];
        $index = view('Hrd::recruitment.tab-rekrut-index',compact('statusApply','lulusan'));
        $tabRekrut = view('Hrd::recruitment.tab-rekrut-diterima',compact('lulusan'));
        $diTerima = view('Hrd::recruitment.diterima');

        return view('Hrd::recruitment.rekrut', compact('query', 'statusApply','index','tabRekrut','diTerima','tanggal','lulusan','no_hp'));
    }
    ...
    public function process_rekrut($id)
    {
        ...
        $test = view('Hrd::recruitment.test_interview');
        $lolos = view('Hrd::recruitment.lolos_interview');

        return view('Hrd::recruitment/process_rekrut', compact('data', 'approve1', 'approve2', 'approve3', 'cek_app1', 'cek_app2', 'cek_app3', 'jadwal_i', 'jadwal_p', 'vacancy','test','lolos'));
    }
