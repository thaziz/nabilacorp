...
function chekJml(id){
  var jmlAwal=angkaDesimal($('.jumlahAwal'+id).val());
  var jmlAkhir=angkaDesimal($('.fQty'+id).val());
  ...
  if(jmlAwal>jmlAkhir){
          iziToast.error({
                ...
                timeout: 2000,
                title: '',
                message: "Ma'af, jumlah barang tidak boleh kurang dari jumlah awal ("+jmlAwal+").",
              });
  }
  return false;
}
   ...
   tablex = $("#tableListToko").DataTable({        
    ...
    function setFormDetail(){
      ...
      var iSalesDetail='';  //isi
          ...
          iSalesDetail+='<td width="6%"><input class="harga'+i_id.val()+' alignAngka form-control" style="width:100%;border:none" name="idt_price[]" value="'+i_price.val()+'" onkeyup="hapus(event,'+i_id.val()+');hitung(\'' + i_id.val() + '\');"  onblur="validationForm();setRupiah(event,\'harga' + i_id.val() + '\')" onclick="setAwal(event,\'harga' + i_id.val() + '\')"></td>';
         ...
         $.ajax({
          url     :  baseUrl+'/penjualan/barang-titipan/chek-qty-return/'+i_id.val()+'/1/1',
          type    : 'GET',           
          dataType: 'json',
          success : function(response){                       
            $('.returnHarga'+i_id.val()).val(SetFormRupiah(response));
...
ctrl = 17;
         $('.move').keydown(function (e) {              
               ...
               var updateQty=0;        
                ...
                var fStok=parseFloat(angkaDesimal($('.fQty'+i_id.val()).val()));
                var a=0;
                var b=0;        
...
function perbaruiData(){
  ...
  var formPos=$('#dataPos').serialize();
     $.ajax({
          url     :  baseUrl+'/penjualan/barang-titipan/update',
          type    : 'GET', 
...
function detail_dt(s_id){  
      var statusPos=$('#s_status').val();
      dataIndex=1;
     $.ajax({      
          url     :  baseUrl+'/penjualan/barang-titipan/'+s_id+'/edit-titipan-dt',
          type    : 'GET',  
          data: {
                "_token": "{{ csrf_token() }}",
                "s_status" :statusPos,
...
function detail($id){  
    $('#detail-titipan').modal('show');
     $.ajax({
          url     :  baseUrl+'/penjualan/barang-titipan/detail/'+$id,
          type    : 'GET',                     
          success : function(response){  
            $('.detail-titipan').html(''); 
