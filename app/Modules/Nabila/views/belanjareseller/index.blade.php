...
<div id="page-wrapper">
    ...
    <div id="title-breadcrumb-option-demo" class="page-title-breadcrumb">
        <div class="page-header pull-left" style="font-family: 'Raleway', sans-serif;">
            <div class="page-title">POS Belanja Reseller</div>
        </div>
        <ol class="breadcrumb page-breadcrumb pull-right" style="font-family: 'Raleway', sans-serif;">
            <li><i class="fa fa-home"></i>&nbsp;<a href="{{ url('/home') }}">Home</a>&nbsp;&nbsp;<i class="fa fa-angle-right"></i>&nbsp;&nbsp;</li>
            <li><i></i>&nbsp;Penjualan&nbsp;&nbsp;<i class="fa fa-angle-right"></i>&nbsp;&nbsp;</li>
            <li><i></i>&nbsp;<a href="{{url('penjualan/POSpenjualan/POSpenjualan')}}">POS Belanja Reseller</a>&nbsp;&nbsp;<i class="fa fa-angle-right"></i>&nbsp;&nbsp;</li>
            <li class="active">POS Belanja Reseller</li>
        </ol>
        <div class="clearfix">
        </div>
    </div>
            ...
            <div class="row mbl">
                <div class="col-lg-12">
                  ...
                  <ul id="generalTab" class="nav nav-tabs">
                    <li class="active"><a id="penjualan" href="#toko" data-toggle="tab">Belanja Reseller</a></li>
                                <li><a id="list" href="#listtoko" data-toggle="tab">List Belanja Reseller</a></li><!-- 
...
@include('Nabila::belanjareseller/includes/modal_alter_status')
@include('Nabila::belanjareseller/js/functions')
@include('Nabila::belanjareseller/js/format_currency')
...
function table(){
  ...
  tablex = $("#tableListToko").DataTable({        
       ...
       serverSide: true,
       ajax: {
          "url": "{{ url("nabila/belanjareseller/listPenjualan/data") }}",
          "type": "get",
          data: {
            "_token": "{{ csrf_token() }}",
            "type"  :"reseller",
...
if($status==true){ 
  ...
  $.ajax({
      url     :  baseUrl+'/nabila/belanjareseller/create',
      type    : 'GET', 
      ...
      success : function(response){    
        ...
        $.ajax({
            url : baseUrl+'/nabila/belanjareseller/printNota/'+response.s_id,
            type: 'get',
            data    :  formPos+'&status='+status,
            success:function (response){
        ...
        /*window.open(baseUrl+'/nabila/belanjareseller/printNota/'+response.s_id,'_blank');*/               
    }
...
function perbaruiData(){
  ...
  var formPos=$('#dataPos').serialize();
  $.ajax({
      url     :  baseUrl+'/nabila/belanjareseller/update',
      type    : 'GET', 
...
function serahTerima(){
   ...
   var formPos=$('#dataPos').serialize();
   $.ajax({
      url     :  baseUrl+'/nabila/belanjareseller/serah-terima',
      type    : 'GET', 
      ...
      success : function(response){    
        ...
        if(response.status=='sukses'){
          ...
          if(response.s_status=='lunas'){
             var childwindow= window.open(baseUrl+'/nabila/belanjareseller/printNota/'+response.s_id,'_blank');            resetFrom();        
         }
...
function detail(s_id){  
  ...
  dataIndex=1;
  $.ajax({
      url     :  baseUrl+'/nabila/belanjareseller/'+s_id+'/edit',
      type    : 'GET',  
      data: {
        "_token": "{{ csrf_token() }}",
        "s_status" :statusPos,
...
function listPenjualan(){
    $.ajax({
      url     :  baseUrl+'/nabila/belanjareseller/listPenjualan',
      type    : 'POST',                     
      data: {
        "_token": "{{ csrf_token() }}"
    },
...
function dataDetailView(s_id,s_note,s_machine,s_date,s_duedate,s_finishdate,s_gross,s_disc_percent,s_disc_value,s_grand,s_ongkir,s_bulat,s_net,s_bayar,s_kembalian,s_customer,c_name,s_status,chek,s_jenis_bayar,s_alamat_cus) {  
    ...
    if(s_net==s_bayar && s_status=='draft' || s_net==s_bayar && s_status=='terima'){              
      $('#serah_terima').removeAttr('disabled');
  }else if (s_net!=s_bayar && s_status=='draft' || s_net!=s_bayar && s_status=='terima'){              
      $('#serah_terima').attr('disabled','disabled');
  ...
  var status='';
  if(s_status=='terima'){
    status='<span class="label label-primary">Terima</span>';
}
if(s_status=='Terima'){
...
$.ajax({
  url     :  baseUrl+'/nabila/belanjareseller/detail-view/'+s_id,
  type    : 'GET',                     
  success : function(response){  
    $('.dataDetail').html(''); 
