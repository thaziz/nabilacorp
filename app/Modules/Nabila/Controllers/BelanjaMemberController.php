...
class BelanjaMemberController extends Controller
{
    ...
    { 
      ...
      /*$data['toko']=view('Nabila::belanjamember/pesanan');      
      $data['listtoko']=view('Nabila::belanjamember/listpesanan');   
      return view('Nabila::belanjamember/pos-pesanan',compact('data'));
*/
      ...
      $data['toko']=view('Nabila::belanjamember/pesanan',compact('machine','paymentmethod','daftarHarga'));      
      $data['listtoko']=view('Nabila::belanjamember/listpesanan');   
      return view('Nabila::belanjamember/index',compact('data','pm','printPl'));
...
$totalBayar=0;
    ...
    function penjualanDtPesanan($id,Request $request){  
      ...
      return view('Nabila::belanjamember/editDetailPenjualan',compact('data','tamp','status'));
    ...
    function penjualanViewDtPesanan($id){            
      ...
      $tamp=array_map("strval",$tamp);      
      return view('Nabila::belanjamember/viewDetailPenjualan',compact('data','tamp'));
      
    }

    function listPenjualanPesanan(Request $request){
      if($request->ajax()){
        return view('Nabila::belanjamember/tableListPesanan');
      }else{
        return 'f';
      }
  ...
  function printNotaPesanan($id, Request $request){
     ...
     return view('Nabila::belanjamember/printNota',compact('data','kembalian','bayar','jumlah','piutang','ttlBayar'));
  ...
  public function delete($id) {
   ...
   public function POSpenjualanPesanan()
    {
      return view('Nabila::belanjamember/pos-pesanan');
    }

    public function find_customer(Request $req) {
      $keyword = $req->keyword;
      $keyword = $keyword != null ? $keyword : '';

      $m_customer = m_customer::take(10);
      if($keyword != '') {
        $m_customer = $m_customer->where('c_name', 'LIKE', "%$keyword%");
      }

      $m_customer = $m_customer->select( DB::raw('c_id AS id'), DB::raw('c_name AS text'), 'c_address' )->get();
      $res = ['m_customer' => $m_customer];
